...
RUNNER=Path('/mnt/data/xsmb_reference_runner_R4_ops_hardened.py')
SOURCE_WB=Path('/mnt/data/XSMB_OPS_HARDENED_FIXTURE.xlsx')
REAL_EVIDENCE=Path('/mnt/data/settlement_evidence_20260920_4source.json')
OUT=Path('/mnt/data/xsmb_OPS_test_output')
OUT.mkdir(exist_ok=True)

spec=importlib.util.spec_from_file_location('xsmb_ops_hardened',RUNNER)
r=importlib.util.module_from_spec(spec); sys.modules['xsmb_ops_hardened']=r; spec.loader.exec_module(r)
...
def set_activation(wb,key,val):
    ...
    governance_keys={
        ...
        'prospective_challenger','spec_hash'
    }
...
def make_base_fixture(dest: Path)->Path:
    ...
    set_activation(wb,'reproducibility_gate','TEST_FIXTURE_OPS_HARDENED')
...
RESULTS=[]
ROOT=Path(tempfile.mkdtemp(prefix='xsmb-ops-tests-',dir='/mnt/data'))
...
def tC07():
 ...
 wb=load_workbook(p); assert any(str(wb['OPERATIONAL_STATE'].cell(rr,1).value or '')=='c07_marker' for rr in range(2,wb['OPERATIONAL_STATE'].max_row+1)); wb.close(); return {'actions':actions}
...
def tC19():
 ...
 assert not mp.exists(); ext=load_workbook(p); kv={str(ext['OPERATIONAL_STATE'].cell(rr,1).value or ''):ext['OPERATIONAL_STATE'].cell(rr,2).value for rr in range(2,ext['OPERATIONAL_STATE'].max_row+1)}; ext.close(); assert kv.get('external_modification')=='EXTERNAL' and 'c19_candidate' not in kv; return err
...
(OUT/'OPS_34_TEST_RESULTS.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
