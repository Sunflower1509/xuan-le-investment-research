#!/usr/bin/env python3
"""R3-HARDENED controlled runner for XSMB MODEL_SPEC v2.1-LIVE-001.
...
def load_workbook_state(workbook_path: str | Path, manifest_dir: str | Path | None = None) -> WorkbookState:
    ...
    try:
        required = {"R5_SUMMARY", "RECONCILIATION_1200", "R5_GATE", "PROSPECTIVE_LEDGER", "MODEL_ACTIVATION", "SPEC_GATE", "REPRO_TEST"}
        missing = sorted(required.difference(wb.sheetnames))
        if missing:
            raise RunnerError("IMPLEMENTATION GATE FAIL — DO NOT FORECAST", "missing workbook sheets", missing)

        activation = _sheet_key_values(wb["MODEL_ACTIVATION"])
...
def _update_activation_field(wb: Any, key: str, value: Any) -> None:
    ...
    for r in range(2, sh.max_row + 1):
        if str(sh.cell(r, 1).value or "") == key:
            sh.cell(r, 2).value = value
            return
    r = sh.max_row + 1
    sh.cell(r, 1).value = key
    sh.cell(r, 2).value = value
...
def run_forecast(workbook_path: str | Path, forecast_date: str, data_cutoff_s: str, manifest_dir: str | Path, test_evidence: str | Path, schedule_evidence: str | Path | None = None) -> dict[str, Any]:
    ...
    with workbook_lock(path, True):
        ...
        wb.close()
        return {"status": "LOCKED_PRE_DRAW", "forecast_id": gate["forecast_id"], "forecast_date": gate["forecast_date"], "data_hash": gate["data_hash"], "probability_sha256": first["probability_sha256"], "top5": first["top5"], "probabilities": first["probabilities"], "forecast_manifest": str(mpath), **commit}
