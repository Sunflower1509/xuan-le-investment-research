...
test("ngoại lệ 17/09 chỉ chấp nhận đúng giá KBS exact-date đã xác minh", () => {
  const cases = [
    ["OIL", 14500, 14400],
    ["VGI", 81800, 81500]
  ];
...
test("cache-token updater chấp nhận URL site.min.js trần sau frontend cleanup", () => {
  ...
  assert.equal(token, "assets/js/site.min.js");
  assert.match(updated, /assets\/js\/site\.min\.js/);
  assert.doesNotMatch(updated, /site\.min\.js\?v=/);
...
test("cache-token updater thay token cũ nhưng fail-closed khi có nhiều site asset", () => {
  ...
  assert.match(updated, /site\.min\.js/);
  assert.doesNotMatch(updated, /site\.min\.js\?v=/);
  assert.throws(
