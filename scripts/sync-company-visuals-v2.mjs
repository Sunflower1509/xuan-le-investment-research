...
const CURL_TIMEOUT_SECONDS = 70;
const CONCURRENCY = 6;
const SCHEMA = "verified-core-asset-webp-v1";
const CANDIDATE_SCHEMA = "civs-candidate-registry-v1";
const USER_AGENT = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/132 Safari/537.36 Xuan-Le-TVS-CIVS-Verifier/2.0";
...
const imageCandidateFromAttrs = (attrs) => {
  ...
  const set = srcsetCandidates(attrs["data-srcset"] || attrs.srcset);
  return set.at(-1) || "";
};
...
const curlText = (url) => execFileSync("curl", [
  ...
  "--user-agent", USER_AGENT, "--header", "Accept: text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", url
  ...
  for (const url of urls) {
    ...
    try {
      const response = await fetch(url, { redirect: "follow", signal: controller.signal, headers: {
        "user-agent": USER_AGENT, "accept": "text/html,application/xhtml+xml;q=0.9,*/*;q=0.8", "cache-control": "no-cache"
      }});
...
const fetchImage = async (entry, imageUrl) => {
  ...
  let fetchError = null;
  try {
    const response = await fetch(imageUrl, { redirect: "follow", signal: controller.signal, headers: {
      "user-agent": USER_AGENT, "accept": ACCEPT_IMAGES, "referer": entry.sourceUrl, "cache-control": "no-cache"
    }});
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
const candidateScore = (candidate, keywords) => {
  ...
  for (const keyword of keywords || []) {
    ...
    if (!key) continue;
    if (hay.includes(key)) score += key.length >= 7 ? 5 : 3;
  }
...
const collectImageCandidates = (html, pageUrl, entry) => {
  ...
  const add = (rawUrl, label, baseScore = 0, context = "") => {
    ...
    if (!url || url.startsWith("data:") || GENERATIVE_STOCK_RE.test(url)) return;
...
const inspectDownloaded = async (entry, downloaded, candidate, tmpDir, pageHits, html) => {
  ...
  if (dimensions.width < MIN_SOURCE_WIDTH || dimensions.height < MIN_SOURCE_HEIGHT || ratio < 0.72 || ratio > 3.4) {
    await fsp.rm(filename, { force: true }); return null;
  }
  ...
  try {
    for (const candidate of candidates.slice(0, 14)) {
      try {
        ...
        if (inspected.length >= 5 && inspected.some((x) => x.qualityScore >= 9 && x.candidate.score >= 5)) break;
    ...
    entry.sourceDiscovery = {
      ...
      strategy: "official-page-embedded-image-v2",
...
const validateSourcePolicy = (entry) => {
  if (!entry?.verified || entry.kind !== "company-asset") throw new Error(`${entry?.ticker || "?"}: visual chưa verified company-asset.`);
  ...
  if (!hostMatchesDomain(imageHost, entry.officialDomain) && !entry.embeddedImageHostVerified && !allowed.has(imageHost)) throw new Error(`${entry.ticker}: external CDN chưa có provenance.`);
};
...
const canReuseExisting = (entry) => {
  if (!entry?.src || !entry.sha256 || !entry.sourceImageUrl || !entry.verified || Number(entry.qualityScore) < 8) return false;
...
const mergeRegistry = (data, registry, logos, research) => {
  ...
  for (const candidate of registry.candidates || []) {
    ...
    if (!ticker || data.visuals[ticker]) continue;
    ...
    data.visuals[ticker] = {
      ...
      sourceDiscovery: { type: "hero-auto", keywords: candidate.keywords || [] },
      fallbackSourceUrls: candidate.fallbackSourceUrls || [],
      alt: `Hình ảnh ${captionSuffix} của ${company} (${ticker}) từ nguồn chính thức`,
      caption: `${company} — ${captionSuffix}`,
      kind: "company-asset",
...
const processOne = async (entry, hashes) => {
  ...
  let downloaded;
  if (entry.sourceDiscovery?.type === "hero-auto" || !entry.sourceImageUrl) downloaded = await discoverHeroAuto(entry);
  else downloaded = await fetchImage(entry, entry.sourceImageUrl);
  ...
  const runners = Array.from({ length: Math.min(CONCURRENCY, entries.length) }, async () => {
    while (true) {
      ...
      try {
        ...
        console.error(`[CIVS FAIL] ${entry.ticker}: ${error?.message || error}`);
      }
    }
  });
...
const run = async () => {
  ...
  if (failures.length) {
    console.error(JSON.stringify({ ok: false, failureCount: failures.length, failures }, null, 2));
    throw new Error(`CIVS full rollout chưa đạt: ${failures.length}/125 mã lỗi. Không ghi dữ liệu production.`);
  }
  ...
  data.meta.rolloutProgressPct = 100;
  data.meta.synced = new Date().toISOString().slice(0, 10);
  data.meta.target = `${TARGET_WIDTH}x${TARGET_HEIGHT}`;
  data.meta.quality = 84;
  data.meta.verification = "CIVS 1.0 COMPLETE: 125/125 visuals validated from first-party official pages/CDNs, Quality Gate >=8/10, normalized locally, SHA-256 audited and fail-closed.";
  fs.writeFileSync(dataPath, `window.COMPANY_VISUALS = ${JSON.stringify(data, null, 2)};\n`);
  console.log(JSON.stringify({ ok: true, schema: SCHEMA, complete: true, coverageTarget: 125, verifiedCount: 125, pendingCount: 0, generated: results.filter((x) => !x.reused).length, reused: results.filter((x) => x.reused).length, results: results.sort((a, b) => a.ticker.localeCompare(b.ticker)) }, null, 2));
};
